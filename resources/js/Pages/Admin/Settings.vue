        ...
        <div class="max-w-6xl space-y-12">
            ...
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="glass p-8 rounded-[40px] border-b-4 border-transparent hover:border-pink-500/30 transition-all group">
                        ...
                        <div class="space-y-4">
                            <div>
                                <label class="block text-[9px] font-black text-text-muted uppercase tracking-widest mb-2">Environment</label>
                                <button @click="form.gateway_bkash_sandbox = !form.gateway_bkash_sandbox"
                                        class="px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all"
                                        :class="form.gateway_bkash_sandbox ? 'bg-pink-600 text-white' : 'bg-white/10 text-text-muted'">
                                    {{ form.gateway_bkash_sandbox ? 'Sandbox' : 'Production' }}
                                </button>
                            </div>
                            <div>
                                <label class="block text-[9px] font-black text-text-muted uppercase tracking-widest mb-2">Merchant Username</label>
                                <input v-model="form.gateway_bkash_username" type="text" placeholder="sandboxTokenizedUser02" class="w-full glass border-none rounded-xl p-3 text-xs font-bold text-adaptive focus:ring-1 focus:ring-pink-500/50 transition-all">
                            </div>
                            <div>
                                <label class="block text-[9px] font-black text-text-muted uppercase tracking-widest mb-2">Merchant Password</label>
                                <input v-model="form.gateway_bkash_password" type="password" placeholder="Merchant password..." class="w-full glass border-none rounded-xl p-3 text-xs font-bold text-adaptive focus:ring-1 focus:ring-pink-500/50 transition-all">
                            </div>
                            <div>
            ...
            <div class="flex justify-end gap-6 pt-4">
                <button @click="saveSettings" 
                    ...
                    <span class="relative z-10">{{ saving ? 'Syncing...' : 'Commit Changes' }}</span>
                </button>
            </div>
        </div>
    </AuthenticatedLayout>
